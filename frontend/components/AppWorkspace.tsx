...
export default function AppWorkspace({initialTab="dashboard"}:{initialTab?:TabId}){
  ...
  const loadDraft=async()=>{try{const r=await api(`/reports/bafa/${rYear}`);if(r?.draft){setDraft(r.draft);setDraftTs(r.updated_at||"");}}catch{}};
  ...
  const saveDraft=async()=>{if(!draft)return;try{await api(`/reports/bafa/${rYear}`,{method:"PUT",body:JSON.stringify({draft})});setDraftTs(new Date().toISOString());toast("ok","Entwurf gespeichert");}catch(e:any){toast("err",e.message);}};
